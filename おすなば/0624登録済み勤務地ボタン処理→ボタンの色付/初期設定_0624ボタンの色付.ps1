...
function drawReturnButton {
    ...
    $ReturnButton.Location = New-Object System.Drawing.Point(235, $Args[0])
...
function drawRegisteredButton {
    ...
    $registeredButton.Location = New-Object System.Drawing.Point(330, $Args[0])
...
:EMPTY for ($i = 0; $i -lt $workPlaceArray.Length; $i++) {
    ...
    drawLabel 20 $koutsukikanLabelLocate 70 "交通機関１：" $forms[$i] | Out-Null
    drawLabel 20 ($koutsukikanLabelLocate + 40) 70 "交通機関２：" $forms[$i] | Out-Null
    drawLabel 20 ($koutsukikanLabelLocate + 80) 70 "交通機関３：" $forms[$i] | Out-Null
    drawLabel 20 ($koutsukikanLabelLocate + 120) 70 "交通機関４：" $forms[$i] | Out-Null
    drawLabel 20 ($koutsukikanLabelLocate + 160) 70 "交通機関５：" $forms[$i] | Out-Null
    drawLabel 20 ($koutsukikanLabelLocate + 200) 70 "交通機関６：" $forms[$i] | Out-Null
    ...
    elseif ($inputContentsResult -eq "No") {
        ...
        # ラベル作成関数呼び出し
        drawLabel 10 10 550 ("【 " + $workPlaceArray[$i] + " 】と同じ勤務地を") $selectForm | Out-Null
        drawLabel 10 27 550 ("登録済みの勤務地から選択してください") $selectForm | Out-Null
        ...
        # Args[2] : OKボタンを表示するフォーム
        # result : OK
        drawOKButton 100 "OK" $selectForm
