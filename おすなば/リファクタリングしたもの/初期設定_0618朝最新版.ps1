...
for ($i = 0; $i -lt $workPlaceArray.Length; $i++) {
    ...
    # #########テスト用？
    $i
    write-host "formcount"
    $formCount
    # #########テスト用？

    # ---------------- Main ----------------- 

    # 初期設定、上書き設定のためのフォーム全体の設定
    # ウィンドウ本体を作っている
    $form = New-Object System.Windows.Forms.Form
    ...
    # フォームを配列に追加
    # #####このフラグはなんだろう。フラグがないと追加しないのか？
    if ($isAdded) {
        $forms += $form
        # ######フォームが追加されたらカウントアップする（どこで使う…?）  
        $formCount++  
    }
    ...
    if ($inputContentsResult -eq "OK") {
        ...
        # if ($inputContentsArray -contains ($workPlaceArray[$i] + "_")) {
        #     $inputContentsArray
        #     write-host 空値あり
        #     $i = $i - 1
        #     $i
        ...
        #         # }
        #         # を押したら、テキストファイルに出力する要素を削除する

        #         #######################################################
        #         $inputContentsArray[3] = ""
        #         $inputContentsArray[2] = $null
        #         $inputContentsArray[1] = "" 
        #         $inputContentsArray[0] = ""     
        #     }
        #     else {
