...
# パイプラインからパラシを取得
# 正規表現でフィルタリング
$pypeKaraUketori = $rowCountnput | ? Name -CMatch 'パラシもどき_.+'
...
$pypeKaraUketori | %{
    ...
    # 3シート目の前に新しいシートを追加し、「目次」と名づける
    $book.worksheets.add($book.sheets(3)) | Out-Null
    $book.sheets(3).name = "目次"
    ...
    for($sheetCount = 4; $sheetCount -le $book.worksheets.count;$sheetCount++){
        ...
        # 目次シートのA列に見出しを張り付け
        $book.worksheets("目次").cells.item($gyouCount,1) = $midashi.text
        # 見出しを太字にする
        $book.worksheets("目次").cells.item($gyouCount,1).font.bold = $true
        $gyouCount++
        ...
        for($rowCount = 1; $rowCount -le 100; $rowCount++){
            ...
            if($sheet.cells.item($rowCount,3).text -cmatch '^[0-9]{1,2}-[0-9]{1,2}'){
                $komidashi = $sheet.cells.item($rowCount,3).text
                 $book.worksheets("目次").cells.item($gyouCount,2) = $komidashi
                 $gyouCount++
            }
        }
    }
